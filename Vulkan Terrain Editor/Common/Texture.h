...
{
public:
    Texture(Vulkan::VulkanDevice* vulkanDevice, QVulkanDeviceFunctions* functions);
...
public:
	...
	*
	*/
    void loadFromFile(const QString& filename, VkFormat format, VkQueue copyQueue, VkImageUsageFlags imageUsageFlags = VK_IMAGE_USAGE_SAMPLED_BIT, VkImageLayout imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, bool forceLinear = false);
	...
	* @param (Optional) imageLayout Usage layout for the texture (defaults VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)
	*/
    void fromBuffer(void* buffer, VkDeviceSize bufferSize, VkFormat format, quint32 width, quint32 height, Vulkan::VulkanDevice* device, VkQueue copyQueue, VkFilter filter = VK_FILTER_LINEAR, VkImageUsageFlags imageUsageFlags = VK_IMAGE_USAGE_SAMPLED_BIT, VkImageLayout imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
};
